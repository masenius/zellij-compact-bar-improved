...
struct State {
    ...
    cwd_names: HashMap<u32, String>,
    git_repo_status: HashMap<u32, bool>,
...
impl State {
    ...
    fn get_smart_name_for_tab(&self, tab: &TabInfo) -> String {
        let Some(terminal_id) = self.active_terminal_pane_id_for_tab(tab.position) else {
            return tab.name.clone();
        };
        let process = self.process_names.get(&terminal_id);
        let cwd = self.cwd_names.get(&terminal_id);
        match (process, cwd) {
            // in a git repo, always show the folder (even while running a process)
            (_, Some(cwd)) if self.git_repo_status.get(&terminal_id) == Some(&true) => cwd.clone(),
    ...
    fn resolve_missing_process_data(&mut self) {
        ...
        for panes in self.pane_manifest.panes.values() {
            for pane in panes.iter().filter(|p| !p.is_plugin && !p.is_suppressed) {
                let terminal_id = pane.id;
                let pane_id = PaneId::Terminal(terminal_id);
                let has_process = !needs_command || self.process_names.contains_key(&terminal_id);
                let has_cwd = !cwd_is_needed || self.cwd_names.contains_key(&terminal_id);
                let has_git_status =
                    !needs_smart || self.git_repo_status.contains_key(&terminal_id);
                ...
                if !has_process {
                    if let Ok(command) = get_pane_running_command(pane_id) {
                        if let Some(process_name) =
                            command.first().map(|c| process_name_from_command(c))
                        {
                            self.process_names.insert(terminal_id, process_name);
                        }
                    }
                }
                if !has_cwd || !has_git_status {
                    if let Ok(cwd) = get_pane_cwd(pane_id) {
                        if !has_cwd {
                            ...
                            self.cwd_names.insert(terminal_id, folder_name);
                        }
                        if !has_git_status {
                            let in_git = self.is_in_git_repo(&cwd);
                            self.git_repo_status.insert(terminal_id, in_git);
                        }
                    }
                }
            }
    ...
    fn apply_pane_format(&mut self) {
        ...
        for pane in panes {
            ...
            let name = self.render_pane_name_for_pane(&format, &pane);
    ...
    fn render_pane_name_for_pane(&mut self, format: &str, pane: &PaneInfo) -> String {
        let terminal_id = pane.id;
        let process = self
            .process_names
            .get(&terminal_id)
        ...
        let cwd = self
            ...
            .cloned()
            .unwrap_or_else(|| pane.title.clone());
        let smart = self.smart_name_for_pane(pane);
    ...
    fn smart_name_for_pane(&self, pane: &PaneInfo) -> String {
        let terminal_id = pane.id;
        let process = self.process_names.get(&terminal_id);
        let cwd = self.cwd_names.get(&terminal_id);
        match (process, cwd) {
    ...
    fn active_terminal_pane_id_for_tab(&self, tab_position: usize) -> Option<u32> {
    ...
    fn handle_cwd_changed(&mut self, pane_id: PaneId, cwd: PathBuf) -> bool {
        ...
        let git_changed = self.git_repo_status.get(&terminal_id) != Some(&in_git);
